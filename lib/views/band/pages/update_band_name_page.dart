...
class UpdateBandNamePage extends StatefulWidget {
  final int bandId;

  const UpdateBandNamePage({super.key, required this.bandId});

  @override
  State<UpdateBandNamePage> createState() => _UpdateBandNamePageState();
}

class _UpdateBandNamePageState extends State<UpdateBandNamePage> {
  ...
  Widget build(BuildContext context) {
    updateBandName() async {
      ...
      final response =
          await BandService.updateBandName(widget.bandId, bandName);
      print('{$jsonDecode(response.body)}');
      if (response.statusCode == 200) {
        final responseBody = jsonDecode(response.body);
        print('밴드 이름 수정 성공: $responseBody');
        Navigator.pop(context, bandName);
